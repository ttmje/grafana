...
func NewRemoteLokiBackend(cfg LokiConfig, metrics *metrics.Historian) *RemoteLokiBackend {
...
func (h *RemoteLokiBackend) RecordStatesAsync(ctx context.Context, rule history_model.RuleMeta, states []state.StateTransition) <-chan error {
	...
	go func() {
		...
		if err := h.recordStreams(ctx, streams, rule.OrgID, logger); err != nil {
			...
			h.metrics.TransitionsFailedTotal.WithLabelValues(fmt.Sprint(rule.OrgID)).Add(float64(samples))
